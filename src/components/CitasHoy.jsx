...
export default function CitasHoy({ lockedBarberId, lockedBarberName }) {
  const [barbers, setBarbers] = useState([]);
  const [appts, setAppts] = useState([]);
  const [form, setForm] = useState({ barberId: lockedBarberId || "", clientName: "", phone: "", time: "" });
  ...
  useEffect(() => {
    if (!lockedBarberId) getBarbers().then((b) => setBarbers(b.filter((x) => x.active))).catch(() => {});
    loadAppts();
  ...
  async function addAppointment() {
    ...
    try {
      await createAppointment({
        barber_id: lockedBarberId || form.barberId || null,
      ...
      setError(e.message || "No se pudo agregar la cita.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function removeAppointment(id) {
    try {
      ...
      setError(e.message || "No se pudo eliminar la cita.");
    }
  }
  ...
  return (
    <Panel>
      ...
      {error && (
        <div style={{ background: "#FBEAE5", border: `1px solid ${T.red}`, color: T.red, borderRadius: 10, padding: "9px 12px", fontSize: 12.5, marginBottom: 10, fontWeight: 600 }}>
          {error}
        </div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        {lockedBarberId ? (
          <div style={{ ...inputStyle(), display: "flex", alignItems: "center", color: T.slate, background: T.bone }}>
            {lockedBarberName || "Tú"}
          </div>
        ) : (
      ...
      <button onClick={addAppointment} style={addBtn} disabled={saving}>
        <PlusCircle size={15} /> {saving ? "Agregando…" : "Agregar cita"}
      </button>
      ...
      <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 8, maxHeight: 320, overflowY: "auto" }}>
        ...
        {sorted.map((a) => {
          const barberName = lockedBarberId ? (lockedBarberName || "Tú") : (barbers.find((b) => b.id === a.barber_id)?.name || "Sin asignar");
          return (
